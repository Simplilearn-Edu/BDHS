...
object SparkStreamingTrendingSubCategoryMonthlyApp {
  ...
  def main(args: Array[String]) {
    ...
    sparkConf.setAppName("Spark Streaming App - TOP " + N + " trending sub category analytics in last " + durationInMinutes + " minutes")
    ...
    val categoryOrderCount = allProducts.map((record) => (record.toString.split(",")(4), record.toString.split(",")(8).toInt)).reduceByKey((i1, i2) => i1 + i2)
    val topN = categoryOrderCount.transform((rdd) => {

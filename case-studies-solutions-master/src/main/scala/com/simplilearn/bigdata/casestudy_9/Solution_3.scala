...
object Solution_3 {
  ...
  def getData(dataset: Dataset[Row]): Unit = {
    val modifiedDataset = dataset
      .withColumn("studentOnBus", UDFUtils.toBoolean(dataset("Number_Of_Students_On_The_Bus"))).drop("Number_Of_Students_On_The_Bus")
      .select("School_Year", "studentOnBus")
      .groupBy("School_Year", "studentOnBus")
      .count()
      .sort(functions.desc("count"))
    System.out.println("Most common reason")
    modifiedDataset.take(5).foreach(
      row => {
        System.out.println("School_Year = "+row.get(0).toString+" Student on Bus = "+row.get(1) +" Accident Count = "+row.get(2))
      }
    )
  }

...
object Solution_4 {
  ...
  def getData(dataset: Dataset[Row]): Unit = {
    var modifiedDataset = dataset
      .filter("industry=='facilities services' and country != ''")
        .select("country")
        .groupBy("country")
        .count()
        .sort(functions.desc("count"))
    System.out.println("Top five countries having maximum companies of facilities services industry.")
    modifiedDataset.take(5).foreach(
      row => {
        System.out.println("Country = "+row.get(0).toString+" Companies Count = "+row.get(1))
      }
    )
  }
